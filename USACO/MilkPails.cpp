...
bool visi[101][101][101];
int x, y, k, m, sol;

void ff(int curx, int cury, int curk) {
    if (visi[curx][cury][curk] || curk > k) return;
    visi[curx][cury][curk] = true;
    ...
    ff(leftoverx, min(cury + curx, y), curk + 1);
    ff(min(cury + curx, x), leftovery, curk + 1);
}
